...
if __name__ == '__main__':
    ...
    select = 'modified_fid_score_'
    num_of_samples = 100
    max_index = 200
    folder_path = 'MNIST_Simulations/FID_path/stage27'
    string_fid = folder_path + '_FID_score'
    string_iters = folder_path + '_FID_score_iters'
    # Check if FID document already generated
    if not os.path.exists(string_fid):
        # load classifier model
        if select == 'modified_fid_score':
            _model = Score_Model()
        ...
        iterations = [int(names.split('_')[1]) for names in os.listdir(folder_path + '/fake_images')]
        iterations.sort()
        score = []
        range_array = []
        ...
        for it in range_array:
            ...
            # real_path = 'MNIST_Simulations/Folder_EdgeOnly_[8, 9]Full13/FID_Images/Iteration_' + str(iterations[it]) \
            #             + '/Real_Images'
            fake_path = folder_path + '/fake_images/Iteration_' + str(iterations[it])
            real_path = folder_path + '/real_images'
            current_index = 0
            mean_score = 0
            k = 0
