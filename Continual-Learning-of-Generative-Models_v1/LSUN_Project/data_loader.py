...
def ensemble(args, instructions):
    dest = 'LSUN_Simulations/ensemble_data_folder' + str(instructions[3])
    ...
    for ind in range(len(instructions[2])):
        ...
        if type(path) is str:
            ...
            G.eval()
            mm = 0
            samp_num = args.ensemble_sample_number
            ...
            for split in range(100):
                generated_image = G(seed[int(split * samp_num / 100):int((split + 1) * samp_num / 100)])
