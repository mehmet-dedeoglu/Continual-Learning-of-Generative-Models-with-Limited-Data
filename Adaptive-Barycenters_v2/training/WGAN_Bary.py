...
class WGAN_GP(object):
    def __init__(self, args, save_folder, G, D, G_optim, D_optim):
        ...
        self.save_all_checkpoints = args.checkpoint_save
    ...
    def check_cuda(self, cuda_flag=False):
        print('Cuda device: ' + torch.cuda.get_device_name(cuda_flag) + ' is being used.')
        print(cuda_flag)
        if cuda_flag == 0 or cuda_flag == 1:
            # self.cuda_index = 1
            self.cuda = True
    ...
    def train(self, train_loader):
        ...
        for epo in range(self.EPOCHS):
            ...
            gen_loss_save.append(self.G_train_step(len(iterator), mone, image_batch, True))
            if epo % self.save_interval == 0 or epo == self.EPOCHS-1:
                # Save checkpoint
                checkpoint_folder = self.save_folder + '/checkpoints'
                if not os.path.exists(checkpoint_folder):
                    os.mkdir(checkpoint_folder)
                if epo != 0:
                    prev_check_file = self.check_file
                ...
                if self.save_all_checkpoints == 'Last':
                    try:
