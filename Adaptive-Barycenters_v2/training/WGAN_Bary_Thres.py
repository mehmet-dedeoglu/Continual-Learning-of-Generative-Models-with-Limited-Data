...
class WGAN_GP_Thres(object):
    def __init__(self, args, save_folder, G, D, G_optim, D_optim, fid_state='Empty'):
        self.G = G
        self.D = D
        self.check_cuda(args.cuda)
        self.batch_size = args.batch_size
        self.d_optimizer_all = D_optim[0]
        self.g_optimizer_all = G_optim[0]
        self.d_optimizer_th = D_optim[1]
        self.g_optimizer_th = G_optim[1]
    ...
    def train(self, train_loader, fid_data, discriminator_save_iter=-1):
        ...
        for epo in range(self.EPOCHS):
            ...
            for _ in range(self.critic_iter-1):
                disc_loss_save.append(self.D_train_step(image_batch, len(iterator), [one, mone], False))
            ...
            for _ in range(self.generator_iters-1):
                gen_loss_save.append(self.G_train_step(len(iterator), mone, False))
            ...
            if epo % self.save_interval == 0:
                ...
                torch.save({
                    'modelG': self.G,
                    'modelD': self.D,
                    'optimizerG_all': self.g_optimizer_all,
                    'optimizerG_th': self.g_optimizer_th,
                    'optimizerD_all': self.d_optimizer_all,
                    'optimizerD_th': self.d_optimizer_th
                }, self.check_file)
    ...
    def generate(self, gen=None, samp_num=1000, inception=False, iter=0, fld_str=''):
        ...
        if gen is None:
            ...
            for split in range(4):
                generated_image = gen(seed[int(split*samp_num/4):int((split+1)*samp_num/4)])
                generated_image = generated_image.mul(0.5).add(0.5).data.cpu()
                generated_image = generated_image * 255
                data = np.asarray(generated_image, dtype="uint8")
                gen_folder = fld_str
    ...
    def D_train_step(self, data_batch, d_num, back, print_bo):
        ...
        for _k in range(d_num):
            ...
            self.d_optimizer_th[_k].zero_grad()
            ...
            self.d_optimizer_th[_k].step()

            # Update remaining parameters
            self.d_optimizer_all[_k].zero_grad()
            d_loss_real = self.D[_k](data_real)
            d_loss_real = self.lamb[_k] * d_loss_real.mean()
            d_loss_real.backward(back[1])
        ...
        return Wasserstein_D.clone().detach().cpu().numpy()
    ...
    def G_train_step(self, d_num, mone, print_bo):
        ...
        self.g_optimizer_th.step()

        self.g_optimizer_all.zero_grad()
        fake_images = self.G(noise)
        for kk in range(d_num-1):
            g_loss = self.D[kk](fake_images)
            g_loss = self.lamb[kk] * g_loss.mean()
