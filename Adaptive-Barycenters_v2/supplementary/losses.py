import torch
import supplementary.utils as utils
...
def calc_eq(x, y, phi, psi, iter_, cost, lambda_eq):
    ux, vy = phi(x), psi(y, iter_)
    return eq_loss(x, y, ux, vy, cost, lambda_eq)


def calc_interp_ineq(x, y, phi, psi, iter_, cost, lambda_ineq, loss=None):
    batch_size = x.size(0)
    x_dim = x.dim()
    ...
    interp_ux = phi(interp_x)
    interp_vy = psi(interp_y, iter_)
