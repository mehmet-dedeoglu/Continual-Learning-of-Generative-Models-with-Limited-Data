...
from Implementation.utils.config import parse_args_fid
import csv
...
def plot_from_txt(data_path, iters_path, fig_texts, args_):
    y = []
    x = []
    ...
    for j in range(0, len(y)):
        ...
        ax.plot(x[j], y[j], linestyle=lines_array[0], marker=markers_array[j+2], linewidth=8, markersize=16)
    ax.set(xlabel=fig_texts[0], ylabel=fig_texts[1], title=fig_texts[2])
    ax.grid()
    fig.savefig(fig_texts[4])
    fig.savefig(fig_texts[5])
    plt.legend(fig_texts[3], fontsize=36)
    plt.show(block=True)
...
if __name__ == '__main__':
    ...
    plot_from_txt(filename_fid, iters, fig_text, args)
