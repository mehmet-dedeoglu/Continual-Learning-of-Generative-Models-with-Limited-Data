...
import shutil
from utils.converterV2 import download_dataset
import supplementary.networks as networks
from archs.wb_networks import CNN_Gen_gp
# from DNN_MNIST import CNN_Gen_gp


def generate_images(arg, instr):
    ...
    folder_name = []
    for idx, chkpoint in enumerate(checkpoints):
        dataset_index = chkpoint[0]
        dataset_sample = instr[1][0][1]
        folder_name.append(score_path + '/Data_folder_' + dataset_index.split('/')[-1])
        if arg.dnn_type == 1:
            move_data(arg, dataset_index, dataset_sample, folder_name[idx], cls_=dataset_index)
...
def move_data(args, cls, samp_num, folder_name, cls_=[]):
    ...
    if not os.path.exists(data_folder):
        download_dataset(data_folder, data=data_folder.split('/')[-1])
    ...
    if type(class_) is not str:
        ...
        print('generating from gan')
        print(class_)
        print('sample number ' + str(samp_num))
        ...
        for split in range(split_number):
            ...
            if args.dataset == 'MNIST':
                for m in range(int(samp_num/split_number)):
                    ...
                    im.save(image_name)
                    im_index += 1
                del generated_image, data
                torch.cuda.empty_cache()
            elif args.dataset == 'CIFAR10' or args.dataset == 'CIFAR10+CIFAR100':
...
def get_data_loader(args, dest):
    ...
    print(str(args.batch_size) + ' ' + str(len(dataset_train)))
