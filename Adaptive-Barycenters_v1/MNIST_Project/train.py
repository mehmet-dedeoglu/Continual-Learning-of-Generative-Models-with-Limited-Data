...
def main(args):
    ...
    while k < len(instructions):
        if instructions[k][0] == 'Pretrain':
            ...
            save_folder = 'MNIST_Simulations/Folder_' + str(tr_cls)
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            save_folder = 'MNIST_Simulations/Folder_EdgeOnly_' + str(tr_cls) + instructions[k][1] \
            ...
            if os.path.exists(save_folder):
                ...
                if instructions[k][1] == 'Ternarized_with_Threshold':
                    g_mod = CNN_Gen_Ter_Thres_gp(1)  # Ternarized model
            ...
            save_folder = 'MNIST_Simulations/Folder_' + str(instructions[k][2])
            if os.path.exists(save_folder):
                ...
                g_mod = CNN_Gen_gp(1)     # Not used due to transfer learning
                g_opt = optim.Adam(g_mod.parameters(), lr=args.learning_rate, betas=(args.beta_1, args.beta_2))
                # Create training object
                ...
                for ind in range(len(instructions[k][2])):
                    ...
                    if type(path) is str:
                        ...
                        dataset.append(gan.generate(gen=G_prev[ind], samp_num=args.sample_number, iter=0,
                                                    fld_str='MNIST_Simulations/data_folder'+str(ind)
                                                            + str(instructions[k][3])))
                    else:
                        G_prev.append(CNN_Gen_gp(1))
            ...
            for kk in range(1, len(instructions[k][2])):
                tr_cls.append(instructions[k][2][kk])
            save_folder = 'MNIST_Simulations/Folder_Aggregate_' + str(instructions[k][2]) + instructions[k][1] \
                          + str(instructions[k][3])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            save_folder = 'MNIST_Simulations/Folder_Transfer_' + instructions[k][1] \
            ...
            if os.path.exists(save_folder):
                ...
                if instructions[k][1] == 'Ternarized_with_Threshold':
                    g_mod = CNN_Gen_Ter_Thres_gp(1)  # Ternarized model (previous model
                    ...
                    g_mod = CNN_Gen_gp(1)  # Full model (previous model parameters will be transferred here.)
                    d_mod = [CNN_Dis_gp(1)]
                ...
                if type(path) is str:
                    ...
                    dataset = [gan.generate(gen=data_G, samp_num=args.sample_number, iter=0,
                                            fld_str='MNIST_Simulations/data_folder' + str(0)
                                                    + str(instructions[k][4]))]
                    dataset_fid = dataset
                else:
                    dataset, _ = get_data_loader_custom(args, args.batch_size, tr_cls, subsample=args.subsample)
            ...
            save_folder = 'MNIST_Simulations/Folder_BaryTransfer_' + str(edge_cls) + instructions[k][1] \
                          + str(instructions[k][4])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                ...
                if instructions[k][1] == 'Ternarized_with_Threshold':
                    ...
                    d_mod = [CNN_Dis_Ter_Thres_gp(1), CNN_Dis_Ter_Thres_gp(1)]
                    ...
                    g_mod = CNN_Gen_gp(1)  # Full model (previous model parameters will be transferred here.)
                    d_mod = [CNN_Dis_gp(1), CNN_Dis_gp(1)]
                ...
                dataset.append(gan.generate(gen=G_prev_data, samp_num=args.sample_number, iter=0,
                                            fld_str='MNIST_Simulations/data_folder'+str(instructions[k][4])))
