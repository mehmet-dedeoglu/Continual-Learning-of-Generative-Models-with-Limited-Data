...
import torch
from DNN_CIFAR100 import CNN_Dis_gp, CNN_Gen_gp
from DNN_CIFAR100_Thres import CNN_Gen_Ter_Thres_gp, CNN_Dis_Ter_Thres_gp
...
def main(args):
    ...
    while k < len(instructions):
        if instructions[k][0] == 'Pretrain':
            ...
            if len(tr_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_Pretrain_Too_Long' + str(tr_cls[:5]) + str(tr_cls[95:-1]) + \
                              instructions[k][1]
            else:
                save_folder = 'CIFAR_Simulations/Folder_' + str(tr_cls)
            # save_folder = 'CIFAR100_Simulations/Folder_' + str(tr_cls)
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            if len(tr_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_EdgeOnly_Too_Long' + str(tr_cls[:5]) + instructions[k][1]
            else:
                save_folder = 'CIFAR_Simulations/Folder_EdgeOnly_' + str(tr_cls) + instructions[k][1]
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                continue
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            if len(tr_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_FullData_Too_Long' + str(tr_cls[:5]) + instructions[k][1]
            else:
                save_folder = 'CIFAR_Simulations/Folder_FullData_' + str(tr_cls) + instructions[k][1]
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                continue
            ...
            save_folder = 'CIFAR_Simulations/Folder_' + str(instructions[k][2])
            if os.path.exists(save_folder):
                ...
                for ind in range(len(instructions[k][2])):
                    ...
                    if type(path) is str:
                        ...
                        dataset.append(gan.generate(gen=G_prev[ind], samp_num=args.sample_number, iter=0,
                                                    fld_str='CIFAR_Simulations/data_folder' + str(instructions[k][3])
                                                            + str(ind)))
                    else:
            ...
            if len(tr_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_Aggregate_Too_Long' + str(tr_cls[:5]) + str(tr_cls[95:-1]) + \
                              instructions[k][1] + str(instructions[k][3])
            else:
                save_folder = 'CIFAR_Simulations/Folder_Aggregate_' + str(instructions[k][2]) + instructions[k][1] \
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            save_folder = 'CIFAR_Simulations/Folder_Transfer_' + instructions[k][1] \
            ...
            if os.path.exists(save_folder):
                ...
                if type(path) is str:
                    ...
                    dataset = [gan.generate(gen=data_G, samp_num=args.sample_number, iter=0,
                                            fld_str='CIFAR_Simulations/data_folder' + str(0)
                                                    + str(instructions[k][4]))]
                    dataset_fid = dataset
                else:
                    dataset, _ = get_data_loader_custom(args, args.batch_size, tr_cls, subsample=args.subsample)
            ...
            if len(edge_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_BaryTransfer_Too_Long' + str(edge_cls[:5]) \
                              + instructions[k][1] + str(instructions[k][4])
            else:
                save_folder = 'CIFAR_Simulations/Folder_BaryTransfer_' + str(edge_cls) + instructions[k][1] \
                              + str(instructions[k][4])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                ...
                dataset.append(gan.generate(gen=G_prev_data, samp_num=args.sample_number, iter=0,
                                            fld_str='CIFAR_Simulations/data_folder' + str(instructions[k][4])))
            ...
            if len(bary_cls) > 75:
                save_folder = 'CIFAR_Simulations/Folder_ModelAverage_Too_Long' \
                              + instructions[k][1] + str(instructions[k][3])
            else:
                save_folder = 'CIFAR_Simulations/Folder_ModelAverage_' + instructions[k][1]\
                              + str(instructions[k][3])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
